...
if (isset($id)) { //jika button hapus diklik
    $exe = mysqli_query($konek, "delete from materi_kursus where id='$id'");
    if ($exe) { //jika berhasil Hapus data
        echo "<script>alert('Data Materi Kursus berhasil Di Hapus!!!')</script>"; //muncul pesan
        echo "<meta http-equiv='refresh' content='0; url=?page=kategori_read'>"; //mengalihkan halaman
    } else { //jika gagal hapus data
        echo "<script>alert('Data Materi Kursus gagal Di hapus!!!')</script>"; //muncul pesan
